...
let pk = "0x56348ca702d4035e814b7b9119132573ed54bf0e3d4df20395131a87b39d134f";
let adminAddress = "0x72829A81F46AdEbE491DC7e3569768DeF559548b";
...
// `beforeEach` will run before each test, re-deploying the contract every
  ...
  beforeEach(async function () {
    ...
    ConversionContract = await ethers.getContractFactory("ConversionV4");
    ...
    await ConversionWallet.updateSignerStatus(adminAddress,true);

  });

// All the testcases of Conversion Contract
describe("Conversion Contract Wrap Signature test", function () {
 
  it("ConversionContract Simple WrapBezoge", async function () {
    const amount = "100000000000000000000000000" 
    const approveAmount = "100000000000000000000000000"
    const usdtBalance = 12

    await BezogeEarth.approve(ConversionWallet.address, approveAmount);

      const balance = await BezogeEarth.balanceOf(owner.address);
      // console.log("Value of balance",balance);
      const balance_  = await balance.toString();
      const userAddress = (owner.address).toString();

      // web3.eth.defaultAccount = adminAddress;

      // const message = web3.utils.soliditySha3(userAddress, balance_);

      // const signature = web3.eth.accounts.sign(message, pk);

      await ConversionWallet.wrapBezogeAdmin("10000000000000000000000");
      await ConversionWallet.adminWithdraw(owner.address);
      await ConversionWallet.wrapBezogeAdmin("10000000000000000000000");
      await ConversionWallet.adminWithdraw(owner.address);
      await ConversionWallet.wrapBezogeAdmin("10000000000000000000000");
      ...
      // ConversionContract.adminWithdraw('0x8b74583B468f604e9AFa4d0f87E5616e4de243da');
...
// // All the testcases of Conversion Contract
// describe("Conversion Contract Wrap Once created signature should be used twice", function () {
...
//     // expect(await ZogiTokenWallet.connect(owner).isAdmin()).to.equal(true);
//     await BezogeEarth.approve(ConversionWallet.address, approveAmount);
...
//       await ConversionWallet.wrapBezoge(balance_,signature.signature);
//       await expect(ConversionWallet.wrapBezoge(balance_,signature.signature)).to.be.revertedWith("Signature is already being used");
...
// // All the testcases of Conversion Contract
// describe("Conversion Contract Wrap Invalid Signature shouldn't be used", function () {
...
//     await BezogeEarth.approve(ConversionWallet.address, approveAmount);
//       pk = "0x56348ca702d4035e814b7b9119132573ed54bf0e3d4df20395131a87b39d1348"
//       const balance = await BezogeEarth.balanceOf(owner.address);
//       const balance_  = await balance.toString();
//       const userAddress = (owner.address).toString();
...
//       await expect(ConversionWallet.wrapBezoge(balance_,signature.signature)).to.be.revertedWith("Invalid Signer");
...
//     await BezogeEarth.approve(ConversionWallet.address, approveAmount);
//     await expect(ConversionWallet.wrapBezoge(amount.toString(),signature.signature)).to
...
//     await BezogeEarth.approve(ConversionWallet.address, approveAmount);
//     await ConversionWallet.wrapBezoge(balanceToWrap.toString(),signature.signature); 
