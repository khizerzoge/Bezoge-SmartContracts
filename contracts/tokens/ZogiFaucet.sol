...
contract ZOGIFaucet is ERC20Snapshot, Pausable,AccessProtected,IZOGI {
    ...
    function faucet(address account_)public whenNotPaused {
        require(account_!= address(0), "ZOGI: Invalid address");
        uint256 amount_ = 100*10**18;
        _mint(account_, amount_);
        emit Mint(account_, amount_);
    }

    function mint(address account_, uint256 amount_)public whenNotPaused onlyAdmin{
        require(account_!= address(0), "ZOGI: Invalid address");
        _mint(account_, amount_);
        emit Mint(account_, amount_);
    }
